...
function getPool() {
    if (typeof window !== 'undefined' || process.env.NEXT_RUNTIME === 'edge') return null;
    try {
        if (!globalForDb.pool) {
            // Only try to require mysql2 if we are NOT in the Edge runtime
            // We use a regular dynamic require inside a check
            const mysql = require('mysql2/promise');
            const { readConfig } = require('./config-server');
